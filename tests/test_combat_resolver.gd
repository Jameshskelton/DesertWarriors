...
func run() -> PackedStringArray:
	...
	var neutral_rowan := UnitState.from_unit_data(DataRegistry.get_unit_data("rowan"), Vector2i(0, 1), "neutral")
	neutral_rowan.set_current_hp(10)
	_assert_true(not resolver.can_unit_attack_from_tile(george, neutral_rowan, george.position), "player units should not attack allied neutral units", failures)
	_assert_true(resolver.can_unit_heal_from_tile(hale, neutral_rowan, hale.position), "player staves should be able to heal allied neutral units", failures)
