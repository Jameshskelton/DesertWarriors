...
func run() -> PackedStringArray:
	...
	var george_triangle_forecast: CombatForecast = resolver.build_forecast(george, brigand, plains, plains)
	...
	_assert_true(george_triangle_forecast.attacker_triangle_state == 1, "sword users should gain weapon triangle advantage over axes", failures)
	_assert_true(plains_forecast.attacker_triangle_state == -1, "axe users should show weapon triangle disadvantage against swords", failures)
	...
	lance_george.item_uses.append(55)
	...
	sword_bram.item_uses.append(55)
	...
	_assert_true(not fragile_result.strikes.is_empty() and int(fragile_result.strikes[0].get("triangle_state", 0)) == 1, "battle strikes should report weapon triangle state", failures)
	_assert_true(not fragile_result.strikes.is_empty() and str(fragile_result.strikes[0].get("triangle_text", "")) == "Weapon Advantage", "battle strikes should report a triangle callout label", failures)
